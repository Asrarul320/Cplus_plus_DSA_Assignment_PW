...
vector<int> findPrefixScore(vector<int>& nums) {
    ...
    int max_val = nums[0];  // Initialize max_val with the first element
    int total_score = 0;    // Initialize total_score
    
    for (int i = 0; i < n; i++) {
        max_val = max(max_val, nums[i]); // Update max_val
        total_score += nums[i] + max_val; // Add current conver value to total_score
        ans[i] = total_score; // Store the result at index i
    }
...
int main() {
    ...
    for (int val : result2) {
        cout << val << " "; // Output: 2 4 8 16 32 64
    }
    cout << endl;
